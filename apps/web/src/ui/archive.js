...
export function createArchiveUI({state,elements,readPlayer,writePlayer,refreshProfiles,renderForms,setError,setStatus}){
  ...
  function paintDirectory(){
    ...
    for(const p of state.playerProfiles||[]){
      ...
      const info=profileInfo.get(p.id),name=el('strong','',p.name);if(p.id===state.activePlayerProfileId)name.append(el('span','current','使用中'));if(info)b.append(assetImage(serverIconUrls(info.server),'server-flag','')||el('span'));b.append(name,el('small','',info?`${{cn:'国服',jp:'日服',en:'国际服',tw:'台服',kr:'韩服'}[info.server]} · ${Object.keys(info.cardList||{}).length.toLocaleString()} 张卡牌`:'读取档案资料…'));
  ...
  document.querySelector('#workspace-profile').onclick=()=>{
    ...
    for(const profile of state.playerProfiles){const b=el('button','switch-item'),copy=el('span'),info=profileInfo.get(profile.id);b.type='button';if(info)b.append(assetImage(serverIconUrls(info.server),'',''));copy.append(el('b','',profile.name),el('small','',info?`${Object.keys(info.cardList||{}).length.toLocaleString()} 张卡牌`:''));b.append(copy);if(profile.id===state.activePlayerProfileId)b.append(el('span','','✓'));b.onclick=()=>{elements.playerProfile.value=profile.id;elements.playerProfile.dispatchEvent(new Event('change',{bubbles:true}));dialog.close();};dialog.querySelector('.switch-list').append(b);}
