...
function cpFullSources(c,trained){if(c.custom)return imageSources(c);return cardArtUrls({card:c.record,illustTrainingStatus:trained}).concat(imageSources({...c,growth:{...cardGrowth(c),illustTrained:trained}}));}
export function createCardDetails({editable=false,onApply,loadCardDetail}={}){
 ...
 function art(){
  ...
  const finish=()=>{if(sequence!==artSequence)return;image.dataset.state='ready';const actual=image.src.includes('after_training')||(!image.src.includes('_normal.')&&!!current.trained);query('.cp-art-status').textContent=!current.custom&&actual!==chosen?'所选原画暂不可用，显示同卡可用图片':'';};
 ...
 function stats(){
  ...
  if(valid&&!values&&!current.custom&&loadCardDetail&&detailState==='idle'){
   detailState='loading';
 ...
 function open(card,{context=editable?'当前档案':'本次计算',trigger}={}){
  ...
  if(c.custom){query('.cp-art-toolbar').hidden=true;query('.cp-release').hidden=true;}
  ...
  if(editable){const skillInput=query('[name=skill]');if(skillInput)skillInput.oninput=skill;query('[data-cp-save]').onclick=()=>{if(![...dialog.querySelectorAll('input[type=number]')].every(n=>n.reportValidity()))return;
    const next={level:Number(query('[name=level]').value),skill:Number(query('[name=skill]')?.value??c.skill),owned:query('[name=owned]').checked,growth:cardGrowth({...c,growth:{...draft,trained:query('[name=trained]').checked,mastery:Number(query('[name=mastery]').value),episodes:[query('[name=episode0]').checked,query('[name=episode1]').checked]}})};
