...
- 单曲候选支配按物理角色拆分为 `FullSkill`、`Captain`、`Filler`；协力独立构造队长池和四个填充位
  候选池，填充位完全不读取技能，只比较综合力和活动 PT 加成；活动加成进入 PT 倍率时，`FullSkill`
  和 `Captain` 使用共享四卡加成状态做分数/加成联合贡献比较；
...
- `FullSkill`：单人、竞演、5v5、CP 挑战等完整队伍技能场景；活动加成进入 PT 倍率时使用联合贡献，
  活动加成进入综合力时沿用原贡献支配；
- `Captain`：协力中的本队队长，技能参与五名玩家的技能排列，并可能成为第六技能；贡献估计额外加入
  `0.1 × 四名外部队友综合力之和`，并与卡片 PT 加成联合比较；
- `Filler`：协力中的其余四张卡，技能完全无效，只按同角色内的 `(综合力, PT 加成)` Pareto 支配，
  相同值保留卡片 ID 最小的规范解。

协力会分别生成 `Captain` 和 `Filler` 两个池，再按角色互异约束组合。队长贡献图严格安全但构造昂贵，
因此使用按 mode 自适应的策略：尚无 incumbent 时用它建立首个严格下界；已有 incumbent 后先使用
不删卡的安全池，当某个 mode 单次产生至少 `10000` 次精确候选后，才对该 mode 的后续道具重建队长
贡献图。这样普通高综合力门槛配置不会承担无收益的重建成本，而低门槛 Mixed 热点可以缩窄共享卡池。

2026-07-31 的任务 Live 协力诊断（135 套道具、20 个 mode、最低综合力 290000）中，原路径需要
`52.40s`，枚举 2355 万叶子并进行 101.87 万次精确调用。自适应队长剪枝与分数分布分层缓存后，结果
保持平均 PT `1528.8`、平均分数 `2900574.8`、综合力 `310006`、卡片
`[1864, 1958, 1988, 2033, 2395]`，release 追踪约 `19.63s`；叶子降至 852 万，精确调用 47.61 万，
其中只有 22.03 万次需要实际构造精确分数分布。

贡献固定点进一步改为分块构图：同角色阶段按角色独立构图；跨角色阶段二分卡池，叶子不超过 128 张时
只建立直接边并跳过传递闭包，父节点复用两侧幸存图、只补左右跨区边后再做子集闭包。所有子图共享父级
完整卡池生成的贡献上下文，避免局部下界过紧。该诊断的预剪枝由约 `3.63s` 降至约 `1.96s`，完整运行
约 `15.39s`；搜索叶子、精确调用、最优队伍和结果均不变。原高门槛诊断实测仍在此前约
`2.48s～4.80s` 的正常波动范围。
...
因此任意两个可行方案 A、B，只要 `E[S_B] <= E[S_A] - D`，就有：

```text
E[PT_B] - 100 <= E[S_B] / D <= E[S_A] / D - 1 < E[PT_A] - 100
```

即平均 PT **严格更低，包含恰好少 18500 的情况**。这里的平均分数是三曲总分的期望；该结论不依赖
技能顺序是否等概率，当前加权洗牌模型同样成立。A 只需是已知可行方案，不必已证明平均分数全局最优。
所以分支的平均总分上界若不高于 `E[S_A] - 18500`，即可安全剪去整支。当前使用精确整数分子时，
对应的带宽为 `18500 × SHUFFLE_PATH_COUNT`；初始带边界保守保留等号，随后可由精确 PT incumbent
进一步收紧。这不是“平均分数只要更低，平均 PT 就更低”，相差不足一个带宽的方案仍需比较精确 PT。

因此期望总分比全局最大期望总分低至少 `D = 18500` 的方案，其平均 PT 必然更低。严格搜索仍按
...
对于活动加成进入 PT 倍率的模式，不再要求替换卡的原始活动 PT 加成单独不低。先从 mode 允许的卡池
枚举“选择四个不同角色”的共享加成总和集合 `B`，然后对候选卡 `c`、每个物理技能场景和仿射上下文
端点比较：

```text
joint(c, B) = (fixed_score_equivalent + score_contribution(c))
              × (10000 + round_bonus(B + card_bonus(c)))
round_bonus(x) = floor((x + 5000) / 10000)
```

所有卡使用同一组 `B`，因此逐点支配关系仍可做传递闭包。硬支配边只有在分数维度不降且卡片 PT
加成也不降时才直接复用；PT 加成较低的候选必须通过联合公式证明可由综合力/技能补偿。`Captain` 的
`score_contribution` 还包含按活动公式折算的 `0.1 × 外部四人综合力`。最终候选仍使用整数分数分布和
正式 PT 公式，不使用联合估计直接决定结果。
...
5v5 在相同分数分布上启用谱面 fever 区间，只把队内排名和胜负作为固定 PT 输入；
...
协力专用缓存把“精确分数分布”和“活动 PT 换算”分层。分数分布键只包含最终个人综合力和本队队长技能
签名；同一分数分布可以跨不同队伍 PT 加成和任务 Live 支援 PT 复用。缓存不保存完整 PT 结果副本，命中
后只重新执行直方图上的活动公式取整，以控制 WASM 内存占用。
...
- 任务 Live 协力与单人一样，由用户输入支援乐队 PT 加成，并在活动 PT 倍率取整后加入。

### 5.3 5v5 多人

- 必须计算 fever。
- 自己的演奏分数使用自己队伍的全部技能。
- 活动 PT 的得分部分只使用自己的分数，不加上其他玩家的分数。
- 用户明确选择队内排名。
- 用户明确选择是否胜利。
- 队内排名和胜负是固定场景输入，因此不需要填写队友预计分数。
- 旧请求中的 `teammateScores` 字段兼容读取但忽略；旧 5v5 历史保留查看，并提示重新计算。
...
`mission_support_pt_bonus` 对任务 Live 的单人和协力模式生效且必须提供；其他活动忽略该
字段并按 `0` 处理。
