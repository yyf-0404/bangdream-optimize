...
“清空游戏缓存”、刷新核心资源和拉取全量资源保留玩家档案与计算历史，仅使旧结果不再参与新计算的缓存复用。“玩家档案缓存”清理仅在网页端提供，会一并删除所有玩家档案与计算历史；执行前必须等待正在进行的计算和配置保存结束。结果页仍可单独删除或清空当前档案的历史。
...
- 当前条数上限：20；
- 排序依据：创建/访问时间；
- 游戏数据刷新、全量同步或清空前，将已有记录持久化为 `reusable: false`：结果与诊断快照保留，可查看，但不能命中新计算；读取旧格式未包含此字段时默认可复用。
- 资源更新过程中返回的旧计算也不能成为可复用缓存。失效标记保存失败时，中止资源更新，不删除历史。
- 清空玩家档案缓存时一并清除结果历史；普通资源操作不删除历史。
