use bangdream_optimize_core::{BuildResult, MaximizeOptions, PlayerConfig, Server};
use bangdream_optimize_data::{DataError, MaximizeInputBuilder, PlayerConfigStore};
use std::sync::Arc;

#[derive(Clone)]
pub struct MaximizeService {
    player_store: Arc<dyn PlayerConfigStore>,
    maximizer: Arc<dyn MaximizeInputBuilder>,
}

impl std::fmt::Debug for MaximizeService {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.debug_struct("MaximizeService").finish()
    }
}

impl MaximizeService {
    pub fn new(
        player_store: Arc<dyn PlayerConfigStore>,
        maximizer: Arc<dyn MaximizeInputBuilder>,
    ) -> Self {
    ...
    pub async fn maximize_for_player(
        &self,
        player_id: i64,
        server: Server,
        event_id: Option<u32>,
        ...
        self.maximize_for_config(player, server, event_id, options)
            .await
    }

    pub async fn maximize_for_config(
        &self,
        player: PlayerConfig,
        server: Server,
        event_id: Option<u32>,
        ...
        self.maximizer
            .maximize(player, server, event_id, options)
            .await
    }
    ...
    pub async fn calculate_for_player(
        ...
        self.maximize_for_player(player_id, server, event_id, options)
            .await
    }
}

pub type OptimizerService = MaximizeService;
...
mod tests {
    ...
    #[async_trait]
    impl MaximizeInputBuilder for StubCalculator {
        async fn maximize(
            &self,
            player: PlayerConfig,
            _server: Server,
    ...
    async fn calculates_for_loaded_player_config() {
        ...
        let result = service
            .maximize_for_player(42, Server::Jp, None, MaximizeOptions::default())
            .await
            .unwrap();
    ...
    async fn reports_missing_player_config() {
        ...
        let err = service
            .maximize_for_player(404, Server::Jp, None, MaximizeOptions::default())
            .await
            .unwrap_err();
